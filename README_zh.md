...
当前为 `V1.8.1-build6` 分支版本，若需查看其他版本，请切换对应分支。
...
- **机内拼接**：`CaptureSetting.INTERNAL_SPLICING`
  - 开：`InternalSplicing.ON`
  - 关：`InternalSplicing.OFF`
...
> 注意：使用MideaSDK之前，必须保证集成了CameraSDK。
...
在CameraSDK的`IPreviewStatusListener.onOpen()`调中显示预览画面
...
您可以将`PlayerGestureListener`置为观察手势操作：
```java
...
`generateHDR`用成功后，outputPath可以作为代理播放或设置为导出url。
...
| 功能   | 模型文件                                                     |
| ------ | ------------------------------------------------------------ |
| 去紫边 | defringe_lr_dynamic_e28c1212.ins<br />defringe_hr_dynamic_7b56e80f.ins |
